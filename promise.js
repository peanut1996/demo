/* 简易版本的promise
    1 共三大块  this.then   resolve/reject   fn(resolve,reject)
    2 this.then负责注册所有的函数   resolve/reject负责执行所有的函数
    3 在resolve/reject里面要加上setTimeout  防止还没进行then注册 就直接执行resolve了
    4 resolve/reject里面要返回this  这样就可以链式调用了
...
function MyPromise(fn) {
  ...
  this.then = function(fulfilled, rejected) {
    ...
    return new MyPromise(function(resolv, rejec) {
      //异常处理
      try {
        if (state == 'pending') {
